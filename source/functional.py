...
def get_current_info_filtration(info_dict):
    """Получение информации из таблицы для вставки её в фильтрацию в онлайн режиме
    info_dict - словарь info_dict = {
    "Федеральный_округ": fed_Distr,
    "Город": city,
    "Область": region,
    "Аббревиатура": univer,
    "ГРНТИ": [GRNTI_1, GRNTI_2], в данный момент ГРНТИ отсутствует
    "Наличие_экспоната": preference_exibit"""

    #info_dict.update({"ГРНТИ": info_dict["ГРНТИ"]})

    select_VUZ = ""; select_Vyst_mo = ""
    ...
    select_VUZ = select_VUZ[0:(len(select_VUZ)-4)]
    select_Vyst_mo = select_Vyst_mo[0:(len(select_Vyst_mo)-4)]
    ...
    #         select_VUZ += key + ", "
    #         if info_dict[key] != '-':
    #             where_VUZ += key + '="' + str(info_dict[key]) + '", '
    #     else:
    #         select_Vyst_mo += key + ", "
    #         if info_dict[key] != '-':
    #             where_Vyst_mo += key + '="' + str(info_dict[key]) + '", '
    # select_VUZ = select_VUZ[0:(len(select_VUZ)-2)]
    # select_Vyst_mo = select_Vyst_mo[0:(len(select_Vyst_mo)-2)]
    # where_VUZ = where_VUZ[0:(len(where_VUZ)-2)]
    # where_Vyst_mo = where_Vyst_mo[0:(len(where_Vyst_mo)-2)]

    # # Получаем информацию из таблицы НИР
    # query = QSqlQuery(f"""SELECT Код_ВУЗа, Аббревиатура FROM Vyst_mo {where_Vyst_mo}""")

    # university_dict = {}
    # while query.next():
    #     current_code = query.value("Код_ВУЗа")
    #     university_dict.update({current_code: query.value("Аббревиатура")})
    # # сортировка по ключам
    # university_dict = dict(sorted(university_dict.items()))
    # # Получаем информацию из таблицы ВУЗов
    # for code_uni in list(university_dict.keys()):
    #     query.exec(
    ...
    # query.exec(
    #             f"""SELECT ГРНТИ FROM Vyst_mo WHERE {where_Vyst_mo}"""
    #         )
    # all_GRNTI = []
    # while query.next():
            #all_GRNTI.append(str(query.value("ГРНТИ")))
    # return university_dict#, all_GRNTI
